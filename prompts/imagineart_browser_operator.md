...
15. Use exactly one click on `Run Selected`; never double-click, never press Enter/Space as a fallback, and never click again because feedback is delayed. Immediately update the ledger status to `clicked_once`, move focus away from the Run button, and treat that selected set as locked until the job is proven finished, failed, or blocked.
...
17. Inspect outputs only after a fresh canvas state confirms completion. Reject moderation placeholders, bad anatomy, fake text, product drift, and incoherent frames. For Seedance/motion moderation or generic generation failure, retry the exact same prompt and same references two more times as isolated one-node launches before simplifying the prompt or returning blocked.
...
29. Before each motion launch, verify the live node panel shows the intended model, duration, quality, ratio, audio setting, non-timed phase prompt, run count, and connected start/end/reference frames. Correct visible settings when they disagree with the written plan. If a motion node has no visible image/start-frame/end-frame/reference connection, do not launch it. If the prompt mentions duration, seconds, or time ranges, remove that language and set the node duration property instead.
30. Run the smallest safe launch transaction. Safe parallel batches are allowed only for independent same-stage nodes with verified model/ratio/duration/inputs/run count, matching credit estimate, no matching Active Runs entries, and a written `batch_id` in `qa/run-ledger.md`. Campaign motion batches may contain at most two independent nodes by default. Retries, locks, Music Studio, ambiguous selections, sequential dependencies, and post-duplicate sessions are one-node launches.
31. If Active Runs shows more jobs than the intended selected-node count, stop launching, mark `duplicate_run_operator_error`, let current jobs finish, switch the rest of the session to isolated one-node launches only, and select only one approved output per role.
...
- Do not retry a launch because the UI did not visibly register the first click. Once the ledger says `clicked_once`, wait, refresh/reopen, inspect Active Runs, or return a blocker.
- Do not batch nodes just because they are visible together. Batch only when the safe batch contract proves they are independent and selected intentionally.
...
- Do not return `blocked: moderation/generation failure` after the first Seedance/motion rejection. Attempt two same-prompt, same-reference isolated retries first. If the failure mentions likeness, real people, private information, or `image_urls`, append `All references are AI generated and do not represent real people.` and retry once before broader simplification. If that still fails, create an isolated Seedance retry that connects only the approved director's-notes/storyboard board as the visual reference and removes photoreal/person/product references. Then attempt one simplified retry if the same failure class persists.
