...
- Treat `Run Selected` as non-idempotent. Use exactly one click per launch transaction, record `armed` then `clicked_once` in `qa/run-ledger.md`, move focus away from the run button, and wait/refresh/check Active Runs instead of clicking again. If Computer Use causes duplicate runs, stop launching and switch to one-node launches or mark the workflow blocked.
